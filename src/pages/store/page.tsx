...
export default function StorePage() {
  ...
  const [activeTab, setActiveTab] = useState<'coupons' | 'menu' | 'reviews'>('coupons');
      ...
      <div className="px-4 pt-6 space-y-6">
        <div className="space-y-3">
          <div className="flex items-start justify-between">
            <h1 className="text-2xl font-sf font-bold text-text">스타벅스 역삼점</h1>
            <div className="flex items-center gap-1">
              <i className="ri-star-fill text-accent text-lg" />
              <span className="text-lg font-sf font-semibold text-text">4.8</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-sf text-text">서울 강남구 역삼동 123-45</span>
              <button className="text-sm font-sf text-primary">길찾기</button>
            </div>
            <div className="flex items-center gap-3">
              <i className="ri-time-line text-text-secondary" />
              <span className="text-sm font-sf text-text">오전 7:00 - 오후 10:00</span>
        ...
        <Card className="border-2 border-primary/20">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-lg font-sf font-semibold text-text">🎯 대표 쿠폰</h3>
            <span className="bg-primary text-white text-sm font-sf font-bold px-3 py-1 rounded-8">
              {coupons[0].discount}
            </span>
          </div>
          <div className="space-y-2 mb-4">
            <h4 className="font-sf font-semibold text-text">{coupons[0].title}</h4>
            <p className="text-sm text-text-secondary font-sf">{coupons[0].description}</p>
            <p className="text-xs text-text-secondary font-sf">{coupons[0].conditions}</p>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 text-sm font-sf">
              <span className="text-accent font-medium">{coupons[0].remaining}개 남음</span>
              <span className="text-text-secondary">{coupons[0].timeLeft} 남음</span>
            </div>
              ...
              {(showAllCoupons ? coupons : []).map((coupon) => (
                <Card 
                  ...
                  className={`cursor-pointer transition-all ${
                    selectedCoupon === coupon.id ? 'border-2 border-primary' : ''
                  }`}
                  onClick={() => setSelectedCoupon(selectedCoupon === coupon.id ? null : coupon.id)}
                >
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-sf font-semibold text-text">{coupon.title}</h4>
                    <span className="bg-accent text-white text-sm font-sf font-bold px-3 py-1 rounded-8">
                      {coupon.discount}
                    </span>
                  </div>
                  <p className="text-sm text-text-secondary font-sf mb-2">{coupon.description}</p>
                  <p className="text-xs text-text-secondary font-sf mb-3">{coupon.conditions}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 text-sm font-sf">
                      <span className="text-accent font-medium">{coupon.remaining}개 남음</span>
                      <span className="text-text-secondary">{coupon.timeLeft} 남음</span>
                    </div>
                    <Button 
                      size="sm" 
                      className="px-6"
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="font-sf font-semibold text-text">{item.name}</h4>
                      ...
                      <p className="text-sm text-text-secondary font-sf mb-2">{item.description}</p>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-sf font-medium text-text">{review.author}</p>
                          <p className="text-xs text-text-secondary font-sf">{review.date}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                    ...
                    <p className="text-sm font-sf text-text leading-relaxed">{review.content}</p>
                  </div>
                </Card>
              ))}
            </div>
          ...
          <div className="relative bg-white rounded-20 p-6 w-full max-w-sm">
            <div className="text-center space-y-6">
              ...
              <div className="bg-primary/5 border-2 border-dashed border-primary/20 rounded-16 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-sf font-semibold text-text">{issuedCoupon.title}</h4>
                  <span className="bg-primary text-white text-sm font-sf font-bold px-3 py-1 rounded-8">
                    {issuedCoupon.discount}
                  </span>
                </div>
                <p className="text-sm font-sf text-text">{issuedCoupon.description}</p>
                <div className="flex items-center justify-between text-sm font-sf">
                  <span className="text-text-secondary">유효기간</span>
                  <span className="text-accent font-medium">{issuedCoupon.timeLeft} 남음</span>
                </div>
              </div>
