      ...
      <div className="pt-20 px-4 space-y-6 pb-20">
        ...
        <div className="grid grid-cols-3 gap-4">
          <Card>
            <div className="text-center space-y-2">
              ...
              <div>
                <p className="text-xl font-sf font-bold text-text">{coupons.filter(c => c.status === 'active').length}</p>
          ...
          <Card>
            <div className="text-center space-y-2">
              ...
              <div>
                <p className="text-xl font-sf font-bold text-text">{coupons.reduce((sum, c) => sum + c.usedCount, 0)}</p>
                <p className="text-xs text-text-secondary font-sf">사용된 쿠폰</p>
              </div>
            </div>
          </Card>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-lg font-sf font-semibold text-text">{coupon.title}</h3>
                      <span className={`text-xs font-sf font-medium px-2 py-1 rounded-full ${getStatusColor(coupon.status)}`}>
                        {getStatusText(coupon.status)}
                      </span>
                    </div>
                    <p className="text-sm text-text-secondary font-sf">{coupon.description}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xl font-sf font-bold text-primary">{coupon.discount}</p>
                  </div>
                </div>

                <div className="flex items-center justify-between text-sm text-text-secondary font-sf">
                  <span>사용: {coupon.usedCount}/{coupon.totalCount}</span>
        ...
        {filteredCoupons.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-sf font-semibold text-text mb-2">쿠폰이 없습니다</h3>
            <p className="text-sm text-text-secondary font-sf mb-4">첫 번째 쿠폰을 만들어 고객들에게 혜택을 제공해보세요</p>
